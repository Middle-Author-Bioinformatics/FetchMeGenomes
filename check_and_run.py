...
FOLDER_PREFIX = 'fetchmegenomes-'
LOG_FILE = '/home/ark/MAB/fetchmegenomes/processed_folders.log'
...
def process_folder(folder_name):
    ...
    local_dir = f"/home/ark/MAB/fetchmegenomes/{folder_name}"
    print(f"Processing folder: {folder_name}")
    download_folder(BUCKET_NAME, folder_name, local_dir)

    # Run fetchmegenomes.sh script with folder_name as argument
    fetchmegenomes_script_path = '/home/ark/MAB/bin/FetchMeGenomes/fetchmegenomes.sh'
    try:
        subprocess.run([fetchmegenomes_script_path, folder_name], check=True)
        print(f"Successfully processed folder {folder_name} using fetchmegenomes.sh.")
    except subprocess.CalledProcessError as e:
        print(f"Error running fetchmegenomes.sh on folder {folder_name}: {e}")
